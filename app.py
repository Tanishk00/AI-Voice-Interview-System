...
def transcribe_full():
    try:
        ...
        if alpha_count < 10:
            return jsonify({
                "success": False,
                "error": "No valid speech detected. Please answer properly."
            }), 400
        ...
        if len(words) < 4:
            return jsonify({
                "success": False,
                "error": "Answer too short. Please explain properly."
            }), 400
        ...
        if unique_ratio < 0.5:
            return jsonify({
                "success": False,
                "error": "Speech unclear or repetitive. Please answer properly."
            }), 400

        #  English-only script check (NO Hindi/Urdu)
        for ch in text:
            if ch.isalpha():
                if not ('a' <= ch.lower() <= 'z'):
                    return jsonify({
                        "success": False,
                        "error": "Please answer in English only."
                    }), 400

        #  Small controlled garbage phrase block
        LOW_INFO_PHRASES = {
            "thank you",
            "thanks",
            "thanks for watching",
        ...
        if text.lower() in LOW_INFO_PHRASES:
            return jsonify({
                "success": False,
                "error": "No valid speech detected. Please answer properly."
            }), 400
       ...
       #---------- LIMIT CHECK OF USER EMAIL ----------
        ...
        try:
            for q in questions:
                ...
                questions_audio_base64.append(encoded_audio)


        except Exception as tts_error:
            print("⚠️ TTS generation failed:", str(tts_error))
            questions_audio_base64= []

        return jsonify({"success": True, "questions": questions,"resume_filename":resume_filename,"questions_audio":questions_audio_base64})
